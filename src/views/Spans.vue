...
import { TraceApi, type SpanBrief } from '@/api'
...
import SpanLogs from './SpanDiagnosis/SpanLogs.vue'
...
const { data, loading, refresh } = useRequest(
  () =>
    traceApi.getSpans(
      ...
      {
        params: {
          'service.name': route.query.service as string,
          'span.name': route.query.span as string,
          'k8s.pod.name': route.query.pod as string
        }
      }
    ),
  {
    ready: () => !!fromTime.value && !!toTime.value,
    refreshDeps: [() => route.query, pagination],
    refreshDepsAction() {
      refresh()
    }
  }
